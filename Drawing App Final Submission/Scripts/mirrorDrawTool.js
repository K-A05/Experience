function mirrorDrawTool() {
	...
	//this changes in the jquery click handler. So storing it as a variable self now means we can still access it in the handler
	var self = this;

	//where was the mouse on the last time draw was called. set it to -1 to begin with
	...
	this.calculateOpposite = function(n, a) {
		if (a != this.axis) {//if the axis isn't the one being mirrored return the same value
			return n;
		}
		if (n < this.lineOfSymmetry) { //if n is less than the line of symmetry return a coorindate that is far greater than the line of symmetry by the distance from n to that line.
			return this.lineOfSymmetry + (this.lineOfSymmetry - n);
		}
		else { //otherwise a coordinate that is smaller than the line of symmetry by the distance between it and n.
			return this.lineOfSymmetry - (n - this.lineOfSymmetry);
		}
	};
	
	//when the tool is deselected update the pixels to just show the drawing and hide the line of symmetry. Also clear options
	...
	//adds a button and click handler to the options area. When clicked toggle the line of symmetry between horizonatl to vertical
	this.populateOptions = function() {
		select(".options").html("<button id='directionButton' class ='button'>Make Horizontal</button>");
		// 	//click handler
